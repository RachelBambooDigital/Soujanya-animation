...
const Sidebar2 = ({ isOpen, toggleSidebar2 }) => {
    ...
    const handleProductClick = (redirectUrl) => {
        const validUrl = redirectUrl.startsWith("http") ? redirectUrl : `https://${redirectUrl}`;
        if (validUrl) {
    ...
    const handleMetaobjectClick = (redirecturlmeta) => {
        if (redirecturlmeta) {
            const validUrl = redirecturlmeta.startsWith("http") ? redirecturlmeta : `https://${redirecturlmeta}`;
            window.location.href = validUrl;
        } else {
                ...
                <div className="relative flex flex-col px-5 lg:px-10 gap-4">
                    <div className="relative border-b w-[100%]">
                        <input
                            ...
                            className="w-full pl-5 pr-3 py-2 bg-transparent text-[gray] focus:outline-none"
                        />
                        <CiSearch className="absolute left-80 top-2 text-lg" />
                    </div>

                    {searchQuery && (
                        <div className="absolute top-full left-0 mt-2 bg-white rounded-md overflow-y-auto z-50">
                            {isLoading ? (
