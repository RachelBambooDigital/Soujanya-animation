...
const ProductDetail2 = ({ language, setLoading }) => {
  ...
  const [useCaseImageUrl, setUseCaseImageUrl] = useState("");
    ...
    const fetchProductData = async () => {
      try {
        const response = await fetch(
          ...
          }/shopify/productDet2/${productId}?targetLanguage=${language}`
        );
        ...
        if (response.ok) {
          ...
          if (data.metafields && data.metafields.edges.length > 0) {
            ...
            const useCaseImage1 = metafields.find(
              (mf) => mf.node.key === "usecaseimage1"
            );

            console.log("Initial benefit image 1:", benefitImage1?.node.value);
            console.log("Initial usecase image 1:", useCaseImage1?.node.value);

            if (benefitImage1) {
              setBenefitImageUrl(benefitImage1.node.value || "");
            }
            if (useCaseImage1) {
              setUseCaseImageUrl(useCaseImage1.node.value || "");
            }
  ...
  return (
    <div
      className="scrollContainer w-full lg:h-[3220px] h-[5500px] overflow-hidden bg-no-repeat"
      ref={svgContainerRef}
    >
      <svg
        width={width}
        ...
        <defs>
          ...
          >
            ...
            <feComposite
              in2="SourceAlpha"
              operator="in"
              result="effect1_backgroundBlur_2834_4224"
            />
            <feBlend
              mode="normal"
              in="SourceGraphic"
              in2="effect1_backgroundBlur_2834_4224"
              result="shape"
            />
          </filter>
          <radialGradient
            id="paint0_angular_2834_4224"
            cx="0"
            cy="0"
            r="1"
