...
const ProductDetail2 = ({language, setLoading}) => {
    ...
    const [cards2, setCards2] = useState({ heading: '', description: '' });
    ...
    const [isComponentLoaded, setIsComponentLoaded] = useState(false); // Track initial load
    const [key, setKey] = useState(0); // Key for forcing re-render
  
    useEffect(() => {
      // Trigger cleanup or necessary setup after initial render
      if (!isComponentLoaded) {
        setIsComponentLoaded(true);
  
        // Force a re-render after the first load to mimic HMR
        setTimeout(() => {
          setKey((prevKey) => prevKey + 1); // Change the key to re-render
        }, 1000); // Small delay to ensure the initial load completes
      }
    }, [isComponentLoaded]);
    ...
    useEffect(() => {
        ...
        const handleOffset = () => {
          ...
          const offset = pathLength - pathLength * percentage * 0.5;
    ...
    }, [key]); // Ensure scroll animation is properly reset after re-render

    useEffect(() => {
        const fetchProductData = async () => {
            try {
                const response = await fetch(
                  `${import.meta.env.VITE_BASE_URL}/shopify/productDet2/${productId}?targetLanguage=${language}`
                );
                ...
                if (response.ok) {
                    ...
                    if (data.metafields && data.metafields.edges.length > 0) {
                      ...
                      console.log("Benefits Title:", benefitsTitle?.node.value);
                      console.log("Benefits Desc:", benefitsDesc?.node.value);
                      ...
                      console.log("Usecase Title:", usecaseTitle?.node.value);
                      console.log("Usecase Desc:", usecaseDesc?.node.value);
    ...
    const handleButtonClick = async (buttonIndex, metafields) => {
      ...
      const useCaseTitleMetafield = `usecasetitle${buttonIndex}`;
      const useCaseDescMetafield = `usecasedesc${buttonIndex}`;
    
      // Find values for both benefits and use cases
      ...
      const useCaseTitle = metafields.find((mf) => mf.node.key === useCaseTitleMetafield)?.node.value;
      const useCaseDesc = metafields.find((mf) => mf.node.key === useCaseDescMetafield)?.node.value;
    ...
    return (
        <div className="scrollContainer w-full lg:h-[3220px] h-[4700px] overflow-hidden bg-no-repeat" ref={svgContainerRef} key={key}>
                    ...
                    <div className='w-full h-screen bg-cover bg-center relative'>
                        ...
                        <div className='absolute inset-0 flex flex-col gap-4 lg:gap-10 justify-end text-white font-medium p-5 xl:p-10'>
                            <div className='grid grid-cols-12 gap-4 max-w-screen-xl lg:pr-20 lg:pl-20'>
                                <div className='col-span-12 lg:col-span-4 bg-opacity-15 backdrop-blur-lg p-5 rounded-md border border-[#bcbcbc]'>
